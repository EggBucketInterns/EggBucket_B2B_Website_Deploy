...
const OutletDetails = () => {
  ...
  const handleSaveEdit = async (formData) => {
    try {
      const response = await fetch(`https://eggbucket-b2b-website-deploy.onrender.com/egg-bucket-b2b/update-outlet/${editingOutlet.id}`, {
        method: 'PATCH',
        // headers: {
  ...
  const handleDeleteClick = async (id) => {
    ...
    if (confirmDelete) {
       ...
       console.log(id)
      try {
        const response = await fetch(`https://eggbucket-b2b-website-deploy.onrender.com/egg-bucket-b2b/delete-outlet/${id}`, {
          method: 'DELETE'
        });
         const data=await response.json()
        if (response.ok) {
