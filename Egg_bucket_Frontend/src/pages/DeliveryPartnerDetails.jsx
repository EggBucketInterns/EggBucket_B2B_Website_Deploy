...
const DeliveryPartnerDetails = () => {
  ...
  const fetchDeliveryPartners = async (query) => {
    try {
      const response = await fetch('https://eggbucket-b2b-website-deploy.onrender.com/deliveryDrivers/egg-bucket-b2b/displayAll-delivery_partner'+query);
  ...
  const handleSaveEdit = async (formData) => {
    try {
      const response = await fetch(`https://eggbucket-b2b-website-deploy.onrender.com/deliveryDrivers/egg-bucket-b2b/delivery_partner/${editingPartner._id}`, {
        method: 'PATCH',
        body: formData
      });
      const data = await response.json();
  ...
  const handleDeleteClick = async (partnerId) => {
    ...
    if (confirmDelete) {
      ...
      try {
        const response = await fetch(`https://eggbucket-b2b-website-deploy.onrender.com/deliveryDrivers/egg-bucket-b2b/delivery_partner/${partnerId}`, {
          method: 'DELETE'
        });
